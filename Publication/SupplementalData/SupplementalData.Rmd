---
title: "Supplemental Data to the article: The MDD concept for establishing trust in non-significant results - a critical review"
author: ""
...
The following R code was used to calculate and plot the power of the MDD dependent on sample size in Figure 3B.  
...
```
Fig. S1. Dependency of MDD power on degree of non-equality between variances and sample size when using the Welch-Satterthwaite approximation to calculate the degrees of freedom for the one-sided t test. Sample sizes are indicated on the right
...
# Trust in non-significant results
...
pMDD | MDD related to control mean | evaluation of false trust/ false mistrust rates
pMDE | MDE with 80% power from post-hoc power analysis related to control mean | evaluation of false trust/ false mistrust rates
pCI | upper bound of CI with 95% confidence level related to control mean | evaluation of false trust/ false mistrust rates
...
Third, experiments were simulated by drawing random samples from a negative binomial distribution creating overdispersed count data using  experimental and real-world parameters as defined above. Each simulated experiment was subsequently analyzed and the results were stored in the output dataframe.
...
## R Code: Figure 4 - False trust and false mistrust rates of pCI, pMDE and pMDD
...
Finally, the pareto-plot (Figure 4 in the manuscript) was drawn using the following R code:
...
Using the same R code as above, we simulated experiments with real effects set to 0.2 vs. zero-effect (Figure S2 A) and 0.8 vs. zero-effect (Figure S2 B), respectively, which led to the following patterns in false trust and false mistrust rates. Note that the general pattern is similar to simulations using an effect size of 0.5 (pMDEs and pMDDs are very similar and both are less optimal than pCIs), but the optimal threshold level (the point along the curve that is nearest to the left bottom corner of the plot) changes and optimal threshold levels may thus have to be set differently for different expected effect sizes to meet regulatory objectives.
...
```
Fig. S2. Rates at which pCI (blue, dashed), pMDE (green, dotted) and pMDD (red, solid, threshold levels in bold italics) erroneously suggest to trust (false trust rate) or mistrust (false mistrust rate) non-significant results dependent on varying threshold levels (numbers along lines). Rates derived from 1000 simulated experiments without effect (zero difference between means) and 1000 experiments with (A) small effect (20% loss in mean abundance) and (B) large effect (80% loss in mean abundance). Values towards the bottom left are more optimal than values towards top right.
  
# Sensitivity of the secondary filter to real effect size 
...
pMDD | MDD related to control mean | secondary filter ability
pMDE | MDE with 80% power from post-hoc power analysis related to control mean | secondary filter ability
pCI | upper bound of 95% CI related to control mean | secondary filter ability
t.mdd | t value corresponding to MDD | check whether calculation of MDD was correct; should equal t.critical
t.critical | critical t value | should equal t.mdd
...
Third, experiments were simulated using  experimental and real-world parameters as defined above. Each simulated experiment was subsequently analysed and the results were stored in the output dataframe.  
...
## R Code: Figure 5 - Correlation of pCI, pMDE and pMDD with real effect sizes 
...
## R Code: Figure 6 - Real effect sizes passing secondary effect size filters
...
```{r eval=FALSE}
# Based on data.frame "nonsig" from simulation 2 (Simulation2-EffectSizeFilter_Figure5.R)
...
# (1) pCI
op <- par(mar = c(3,8,1,1), oma = c(0.5,0.5,0,0), cex = 0.8)

plot(summ.pCI$max ~ summ.pCI$threshold, type ="n", 
     ylim = c(0,1.03), xlim = rev(c(30,100)),
     las = 2, xlab = "", ylab = "", las = 1, bty ="l", 
     yaxt = "n", xaxt = "n")
axis(side = 1)
axis(side = 2, las = 2, labels = T, at = seq(0, 1, by =0.1), line = 4)
...
lines(smooth.spline(summ.pCI$median ~ summ.pCI$threshold), 
      lty = 1, lwd =2, col = "darkblue")
...
mtext(c("p-value filter","max", "75%", "median", "25%", "min"), 
      ...
      at = c(1.03,max(nonsig$effectsize)-0.01, quantile(nonsig$effectsize, 0.75),
             median(nonsig$effectsize), quantile(nonsig$effectsize, 0.25),
             min(nonsig$effectsize)))
...
lines(smooth.spline(summ.pMDE$median ~ summ.pMDE$threshold), 
...
lines(smooth.spline(summ.pMDD$median ~ summ.pMDD$threshold), 
