...
ui <- fluidPage(
  ...
  tabsetPanel(
      tabPanel("The MDD – a critical review", fluid = TRUE,
        mainPanel(
          HTML(paste("This Shiny application accompanies the article: ", "",
                     "The MDD concept for establishing trust in non-significant results – a critical review",
                     sep="<br/>"))
        )
      ),
   # Tab2 - pareto================
      tabPanel("I. Discrimination between true and false negatives", fluid = TRUE,
        sidebarLayout(
          sidebarPanel(
          ...
          mainPanel(
            ...
            "Figure 1. The translation of continuous indicators into binary decisions requires choosing a threshold level at which a non-significant result is interpreted as zero-effect (trust non-significant result). This translation is accompanied by some error (false trust and false mistrust rates). The plot shows the rates at which pCI (blue, dashed), pMDE (green, dotted) and pMDD (red, threshold levels in bold italics) erroneously suggest to trust (false trust rate) or mistrust (false mistrust rate) a non-significant result, when decisions are based on varying threshold levels (numbers along lines). Rates are calculated from 1000 simulated experiments without effect (zero difference between means) and 1000 experiments with effect (according to the selected effect size). Values nearer to the bottom left are pareto-superior (more optimal) to values nearer to the top right (less optimal)."
          )
        )
      ),
   ...
   # Tab3 - effect size filter====================
        tabPanel("II. Sensitivity to real effect size", fluid = TRUE,
           ...
           sidebarLayout(
             ...
             mainPanel(
               plotOutput("corrPlot"),
               "Figure 1. Correlation between pCI (A), pMDE (B) and pMDD (C) and real effect size in experiments with nonsignificant test outcomes (total number of simulated experiments = 1000). Each dot represents one simulated experiment. Red solid line : regression line, grey dashed line: estimator equals real effect (proportional difference in distribution means as set for simulation of experiment), grey squares: experiments in which the indicator is lower than the real effect.",
               plotOutput("filterPlot"),
               "Figure 2. Distribution of real effect sizes from experiments passing, both,  the significance filter (p < 0.05 excluded; grey lines in all three plots) and the secondary statistical filters pCI, pMDE and pMDD when applying different thresholds (ranging from 100 to 30). Lines represent median (solid line), 25% and 75% quartiles (dashed lines) and minimum and maximum real effect size (dotted lines)."
             )
           )
        )
      )
...
# observeEvent: generate1 - effect size filter --------------------
